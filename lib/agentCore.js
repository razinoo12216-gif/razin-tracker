...
// Mirrors recurrenceMatches() in src/app.js. Recurring rows are templates: their `day`
// is a start date, not a due date. Weekly format is day-of-week numbers separated by
// spaces or commas ("1,3,5"); monthly is "monthly-<dayOfMonth>".
function recurrenceMatches(recurrence, dayISO) {
  if (!recurrence || recurrence === 'none') return false;
  if (String(recurrence).startsWith('monthly-')) {
    return parseInt(dayISO.split('-')[2], 10) === parseInt(String(recurrence).split('-')[1], 10);
  }
  const p = dayISO.split('-');
  const dow = new Date(Date.UTC(+p[0], +p[1] - 1, +p[2])).getUTCDay();
  return String(recurrence).split(/[ ,]+/).map(Number).includes(dow);
}
...
const TABLE_MAP = {
  tasks: 'Daily task list ("12 Ticks" tab). day, title, time, done, recurrence, category, notes. IMPORTANT: a row with a non-empty `recurrence` is a TEMPLATE — its `day` is the start date, not a due date, and the app generates a virtual instance for each matching day. A recurring row with an old `day` is NOT overdue. Use the listTasks tool rather than counting this table by hand.',
...
async function execTool(ctx, name, input) {
  ...
  if (name === 'listTasks') {
    // RECURRENCE FIX 2026-08-08. This used to classify anything with day < today as
    // overdue, ignoring the recurrence column entirely. Recurring tasks are TEMPLATES
    // carrying their original start date, so all 44 of Razin's open dailies were being
    // reported as overdue — the agent kept announcing "22 overdue" when the true count
    // of genuinely late one-off tasks was ZERO. A false alarm repeated daily is worse
    // than no alarm: it trains him to ignore the list.
    const rows = await read('tasks', 'done=eq.false&select=id,title,day,time,category,notes,recurrence&order=day.asc&limit=300');
    const overdue = [], today = [], upcoming = [], undated = [], recurringDue = [], recurringOther = [];
    rows.forEach(r => {
      const rec = r.recurrence && r.recurrence !== 'none' ? r.recurrence : null;
      if (rec) return (recurrenceMatches(rec, t) ? recurringDue : recurringOther).push(r);
    ...
    return {
      ...
      counts: {
        ...
        undated: undated.length, recurring_due_today: recurringDue.length,
        recurring_total: recurringDue.length + recurringOther.length,
      },
      overdue: overdue.slice(-25),
      today,
      recurring_due_today: recurringDue,
      upcoming: upcoming.slice(0, 25),
      undated: undated.slice(0, 15),
      _note: 'overdue = genuinely late ONE-OFF tasks only. Recurring items are habits on a schedule, ' +
             'never overdue — do not add them to an overdue count or report them as late.',
    };
  }
...
async function assembleContext(ctx) {
  ...
  const rules = [
    ...
    'EXECUTION AND MONEY COME FIRST. Razin is out working, driving and dealing with real life most of the day. Lead every answer with work, deals, money and the people he needs to move — that is what he opens this app for. Goals, targets and discipline are a LIGHT NOTE at the end, one or two lines, never the headline and never a lecture.',
    'DO NOT EXPECT HIM TO LOG THINGS. He is not going to stop mid-day to tick a box, and a system that depends on him doing so has already failed. Instead, ASK. Short, specific, answerable in five words while driving: "Gym today or tomorrow?", "Eaten yet — what?", "Did E come back to you?". Then capture the answer yourself with logDay / logTarget / addCommitment. His reply IS the log. One question at a time, never a checklist.',
    'When you ask about food or training, the point is helping him decide what to do NEXT, not auditing what he failed to record. "You have not eaten since this morning and you train at 6 — get protein in now" is useful. "You did not log your macros" is worthless.',
...
// MODE: CAPTURE (2026-08-10). Replaces the removed Chat tab as the way anything gets logged.
// Razin is on his phone, one-handed, possibly at a red light. He types a fragment and hits Send.
// The whole job is: turn the fragment into rows, then get out of the way. Anything that reads as
// conversation is a failure here — he did not open a chat, he opened a capture box.
const CAPTURE_DIRECTIVE = '\n\nMODE: QUICK CAPTURE.\n' +
  'Razin has typed a short fragment into a capture box on his phone. It is not a question and it ' +
  'is not the start of a conversation. Your entire job is to write down what he said.\n' +
  'Rules:\n' +
  '- WRITE FIRST. Call the tools before you write a word of prose. Never ask him to confirm, never ' +
  'ask which table it belongs in, never offer to log it — just log it.\n' +
  '- Discipline mentions (fajr, gym, wake, sleep, clean/pmo, prayers, focused hours) -> logDay. ' +
  'One call covers all of them; do not call it once per item.\n' +
  '- A promise in either direction -> addCommitment. "I told E I would send X by Tuesday" is ' +
  'i_owe. "E said he would release the funds Monday" is owed_to_me. Record it even with no date.\n' +
  '- Something he wants to remember to do -> addTask.\n' +
  '- A named daily target -> logTarget.\n' +
  '- A durable fact about a company, deal or person with no action attached -> remember.\n' +
  '- One fragment can be several of these at once. Do all of them in one turn.\n' +
  '- If the fragment is genuinely ambiguous, pick the most likely reading, write it, and say which ' +
  'reading you took in your one line. A logged guess he can correct beats nothing logged.\n' +
  'REPLY FORMAT: at most 12 words, plain text, no markdown, no greeting, no sign-off, no follow-up ' +
  'question. State only what you wrote, e.g. "Gym + fajr logged. Commitment to E due Tue 11 Aug." ' +
  'The app shows him the verified receipt underneath, so do not restate it in detail.';
...
export async function runAgent({ messages, env, mode, readOnly = false, cheap = false, maxTokens }) {
  ...
  if (mode === 'capture') system += CAPTURE_DIRECTIVE;
