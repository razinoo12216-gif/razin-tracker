...
export async function runAgent({ messages, env, mode, readOnly = false, cheap = false }) {
  const read = db(env);
  ...
  for (let turn = 0; turn < MAX_TOOL_TURNS; turn++) {
    ...
    const resp = await callAnthropic(env, { model: cheap ? CHEAP_MODEL : MODEL, max_tokens: MAX_TOKENS, system: sys, tools: activeTools, messages: msgs });
    if (resp.usage) {
