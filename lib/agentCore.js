...
const CLAIM_VERBS = /\b(?:added|logged|ticked|un-?ticked|created|saved|stored|remembered|deleted|removed|updated|edited|scheduled|marked|noted\s+—\s+logged)\b/i;

// True if the prose reads as "I changed something in your data".
// Deliberately errs toward firing: a false positive costs one cheap correction
// turn, a false negative costs Razin's trust in the whole system.
export function claimsWrite(text) {
  if (!text) return false;
  for (const raw of String(text).split(/[\n.!?]+/)) {
    const line = raw.trim();
    if (!line) continue;
    // strip clauses that are about HIM, about the future, or explicit negations
    const cleaned = line
      .replace(/\b(?:you|he|she|they|razin)\b[^,;]*/gi, '')
      .replace(/\b(?:i can|i could|i will|i'?ll|shall i|want me to|do you want|if you|should i|i'?d)\b.*/gi, '')
      .replace(/\b(?:not|never|didn'?t|haven'?t|hasn'?t|couldn'?t|can'?t|wasn'?t|weren'?t|no)\s+\S+\s*\S*/gi, '')
      .replace(/\bnothing\b.*/gi, '');
    if (CLAIM_VERBS.test(cleaned)) return true;
    if (/^(?:done|noted)\b/i.test(line) && !/\bnot\b/i.test(line)) return true;
  }
  return false;
}
...
const CORRECTION_PROMPT =
  'AUTOMATED INTEGRITY CHECK — this is not from Razin, do not mention it to him.\n' +
  'Your last reply reads as if you changed something in his data, but NO write tool ' +
  'executed this turn and nothing was saved. Exactly one of these is true:\n' +
  '(a) He asked for a change and you skipped the tool call. Make the tool call NOW, ' +
  'this turn, for real — then answer.\n' +
  '(b) No change was requested, or the thing he mentioned is not a task. Answer again ' +
  'with no wording that implies anything was saved, added, logged, ticked or remembered. ' +
  'If he told you a real-world fact and it is worth keeping, call `remember` for real ' +
  'instead of saying "logged".\n' +
  'No apology, no meta-commentary. Just do it.';
...
async function finish(ctx, { text, toolsUsed, usage, stop_reason }) {
  ...
  let out = text || '';
  if (!checked.length && claimsWrite(out)) {
    out = '⚠️ NOTHING WAS WRITTEN. The reply below claims a change was made, but no ' +
          'write tool ran this turn and nothing reached the database. Treat it as NOT done.\n\n' + out;
  }
...
export async function runAgent({ messages, env, mode }) {
  ...
  for (let turn = 0; turn < MAX_TOOL_TURNS; turn++) {
    ...
    if (text) {
      ...
      if (!ctx.actions.length && corrections < 1 && claimsWrite(text)) {
        corrections++;
    ...
    if (resp.stop_reason === 'max_tokens') {
      return await finish(ctx, { text: '⚠️ The reply was cut off before any text was produced (hit the ' + MAX_TOKENS + '-token output cap). Ask a narrower question.', toolsUsed, usage, stop_reason: 'max_tokens' });
    }
